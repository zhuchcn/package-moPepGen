...
def add_subparser_call_variant(subparsers: argparse._SubParsersAction):
    ...
    p: argparse.ArgumentParser = subparsers.add_parser(
        ...
        description='Genomic variant data must be generated by one of the moPepGen parsers. See moPepGen --help',
    ...
    p.add_argument(
        ...
        required=False,
        default=None,
        help='Directory path that graph data are saved to. Graph data are not saved if this is not given.',
        metavar='<file>'
    )
    p.add_argument(
        '--max-adjacent-as-mnv',
    ...
    p.add_argument(
        ...
        action='store_true',
        help='Include peptides of selenoprotiens that the UGA is treated as termination instead of Sec.'
    )
    p.add_argument(
        '--w2f-reassignment',
        action='store_true',
    ...
    p.add_argument(
        ...
        type=int,
        help='Maximal number of variants per node. Useful when local regions are heavily mutated. When creating the cleavage graph, nodes containing variants larger than this value are skipped. Set -1 to disable.',
        default=[7],
        nargs='+',
        metavar='<number>'
    )
    p.add_argument(
        ...
        type=int,
        help='Additional variants allowed for every miscleavage. Used with --max-variants-per-node to handle hypermutated regions. Set -1 to disable.',
        default=[2],
        nargs='+',
        metavar='<number>'
    )
    ...
    p.add_argument(
        ...
        type=int,
        help='When making the cleavage graph, the minimal number of nodes to trigger pop collapse.',
        default=30,
        metavar='<number>'
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        action='store_true',
        help='Process only noncanonical transcripts of fusion transcripts and circRNA. Canonical transcripts are skipped.'
    )
    p.add_argument(
        '--timeout-seconds',
        type=int,
